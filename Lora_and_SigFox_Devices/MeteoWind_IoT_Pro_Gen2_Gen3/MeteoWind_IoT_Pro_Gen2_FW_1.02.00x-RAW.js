//MeteoWind_IoT_Pro_Gen2 periodic payload decoder - output of speeds are in RAW Hz, so you need to recalc it to m/s
function decodeUplink(input) {
    ...
    var wind_ave = hz_avg > 0 ? precisionRound(hz_avg, 2) : 0;

    var hz_3sgust = hz_avg + precisionRound(bitShift(9)*0.1, 2);
    var wind_3sgust = hz_3sgust > 0 ? precisionRound(hz_3sgust, 2) : 0;

    var hz_1sgust = hz_3sgust + precisionRound(bitShift(8)*0.1, 2);
    var wind_1sgust = hz_1sgust > 0 ? precisionRound(hz_1sgust, 2) : 0;

    var hz_3min = precisionRound(bitShift(9)*0.1, 2);
    var wind_3smin = hz_3min > 0 ? precisionRound(hz_3min, 2) : 0;
    ...
    var deg_ccw_min = precisionRound(bitShift(9)*1, 1);
    var deg_ccw_max = precisionRound(bitShift(9)*1, 1);
	...
	"00A_device" : "MeteoWind IoT Pro Gen2",
        ...
        "wind_ave_hz": wind_ave,
        "wind_3s_gust_hz": wind_3sgust,
        "wind_1s_gust_hz": wind_1sgust,
        "wind_3s_min_hz": wind_3smin,
        "wind_stdev": hz_stdev,
        "dir_ave": deg_1s_avg,
